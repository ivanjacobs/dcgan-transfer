...
class DCGAN(object):
    def __init__(self, sess, image_size=108, is_crop=True,
                 ...
                 checkpoint_dir=None, sample_dir=None, transfer=0):
        """
        ...
        self.pretrained_model = checkpoint_dir.split('/')[-1]
        self.build_model()
    ...
    def train(self, config):
        ...
        print 'self.transfer: '+str(self.transfer)
        if self.transfer > 0:
            s = self.output_size
            ...
            '''
            # reset layer 4
            self.h4_w, self.h4_b = reset_layer([self.batch_size, s2, s2, self.gf_dim*1], 
                                                   [self.batch_size, s, s, self.c_dim])

        print 'config.epoch: '+str(config.epoch)
        for epoch in xrange(config.epoch):
            if config.dataset == 'mnist':
                ...
                print 'config.train_size: '+str(config.train_size)
    ...
    def load(self, checkpoint_dir):
        ...
        model_dir = "%s_%s_%s" % (self.pretrained_model, self.batch_size, self.output_size)
